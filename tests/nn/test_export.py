...
def create_zero_action_model() -> ActorCriticAgent:
    ...
    class TestActorModel(GaussianActionModel):
        ...
        def __call__(self, obs: FrozenDict[str, Array], cmd: FrozenDict[str, Array]) -> Array:
            ...
            print(f"DEBUG - raw action output (before any potential normalization): {result[0, :3]}")
...
def test_export_and_inference():
    ...
    print(f"joint_position_observation mean: {variables['normalization']['obs_mean_joint_position_observation']}")
    print(f"joint_position_observation std: {variables['normalization']['obs_std_joint_position_observation']}")
    print(f"joint_velocity_observation mean: {variables['normalization']['obs_mean_joint_velocity_observation']}")
    print(f"joint_velocity_observation std: {variables['normalization']['obs_std_joint_velocity_observation']}")
    ...
    test_obs = {
        ...
        "joint_position_observation": jnp.ones((1, NUM_OUTPUTS)) * 30.0,  # Should normalize to (30-1)/2 = 14.5
        "joint_velocity_observation": jnp.ones((1, NUM_OUTPUTS)) * 5.0,  # Should normalize to (5-1)/2 = 2.0
    ...
    test_obs2 = {
        ...
        "imu_acc_sensor_observation": jnp.ones((1, 3)) * 3.0,
        "imu_gyro_sensor_observation": jnp.ones((1, 3)) * 3.0,
        "joint_position_observation": jnp.ones((1, NUM_OUTPUTS)) * 11.0,  # Should normalize to (11-1)/2 = 5.0
        "joint_velocity_observation": jnp.ones((1, NUM_OUTPUTS)) * 9.0,  # Should normalize to (9-1)/2 = 4.0
        "base_orientation_observation": jnp.ones((1, 3)) * 3.0,
        "base_linear_velocity_observation": jnp.ones((1, 3)) * 3.0,
        "base_angular_velocity_observation": jnp.ones((1, 3)) * 3.0,
    }
    ...
    if is_close and is_expected:
        print("✅ Test passed! The exported model correctly applies normalization and preserves action scaling.")
    else:
        print("❌ Test failed! The exported model does not correctly handle normalization or action scaling.")
        if not is_close:
...
def test_action_normalization():
    ...
    print(f"Normalized joint_position_observation: {normalized_obs['joint_position_observation'][0, :3]}")
    print(f"Normalized joint_velocity_observation: {normalized_obs['joint_velocity_observation'][0, :3]}")
